...
/**
 * 출퇴근 근접도 (0~1)
 * 직주근접 서비스의 핵심 지표. 짧은 구간에서 급격히 감소해
 * 5분과 15분 차이가 점수에 의미 있게 반영되도록 설계.
 *   5분 이하 → 1.00
 *  10분     → 0.92
 *  15분     → 0.82
 *  20분     → 0.70
...
export function commuteScore(minutes) {
  ...
  if (minutes <= 10) return 1.00 - ((minutes - 5)  / 5)  * 0.08; // 1.00 → 0.92
  if (minutes <= 15) return 0.92 - ((minutes - 10) / 5)  * 0.10; // 0.92 → 0.82
  if (minutes <= 20) return 0.82 - ((minutes - 15) / 5)  * 0.12; // 0.82 → 0.70
  if (minutes <= 30) return 0.70 - ((minutes - 20) / 10) * 0.20; // 0.70 → 0.50
  if (minutes <= 45) return 0.50 - ((minutes - 30) / 15) * 0.25; // 0.50 → 0.25
  if (minutes <= 60) return 0.25 - ((minutes - 45) / 15) * 0.17; // 0.25 → 0.08
  if (minutes <= 90) return 0.08 - ((minutes - 60) / 30) * 0.08; // 0.08 → 0.00
  return 0;
}
