...
import { calcLoanInterest } from './loan';
import { CANDIDATE_REGIONS, formatKRW } from '../components/shared';

// 세전 연봉 → 점수 계산용 월 소득 근사 (÷12 후 세후 0.87)
export function annualToMonthlyNet(annualIncome) {
  return annualIncome > 0 ? Math.round((annualIncome / 12) * 0.87) : 0;
}

// 구버전 히스토리는 income을 "월 소득"으로 저장했음 — 세전 연봉이 1,000만원 미만일 리 없으니 보정
export function normalizeAnnualIncome(income) {
  const n = Math.round(income || 0);
  return n > 0 && n < 1000 ? n * 12 : n;
}
...
export async function buildResults({ asset, income, workLat, workLng, loan, loanType = '버팀목', loanRate, generalRate, transport }) {
  const wx = workLng || 126.9780;
  const wy = workLat || 37.5665;

  const annualIncome = normalizeAnnualIncome(income);
  const monthlyIncome = annualToMonthlyNet(annualIncome); // 점수·감당가능 판정용
  ...
  candidatePool.forEach((region, idx) => {
    ...
    for (const opt of dynamicOptions) {
      ...
      const li = calcLoanInterest({
        deposit, asset, annualIncome, loanType,
        policyRate: loanRate || 3.5,
        generalRate,
      });
      if (loan && !li.coverable) continue; // 전세가 80% 넘게 빌려야 함 → 대출로도 불가
      const loanNeeded = li.loanAmount;
      const monthlyInterest = li.monthlyInterest;
      const loanNote = li.note;
      const monthlyMan = monthlyInterest + rentMan + (region.maintenanceFee || 0);

      if (monthlyIncome > 0 && monthlyMan > monthlyIncome * MAX_AFFORDABLE_RATIO) continue;

      const commuteMin = transport === '자가용' ? carMin : transitMin;
      const cs = commuteScore(commuteMin);
      const ps = priceScore(monthlyMan, monthlyIncome);
      ...
      const transitLabel = `${transitEstimated ? '약 ' : ''}${transitMin}분`;
      const carLabel = `${carEstimated ? '약 ' : ''}${carMin}분`;
